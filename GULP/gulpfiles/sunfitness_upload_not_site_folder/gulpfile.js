...
var user = process.env.FTP_USER,
    ...
    host = '31.220.16.225',
    port = 21,
    localFilesGlob = ['../../OpenServer/domains/sunfitness.cz/wp-content/themes/interface/*'],
    //localFilesGlob = ['*'],
...
gulp.task('ftp-deploy-watch', function() {
    ...
    .on('change', function(event) {
      ...
      console.log('Changes detected! Uploading file "' + remoteFolder + '", ' + event.type);
      return gulp.src( [remoteFolder], { base: '.', buffer: false } )
        .pipe( conn.newer( remoteFolder ) ) // only upload newer files 
        .pipe( conn.dest( remoteFolder ) )
      ;
    });
